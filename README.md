...
- **Why history starts on 4 Sep 2026**: Piecebook only knows the prices it has recorded itself, and it started recording on 4 Sep 2026. There is no free historical feed: Collectr and PriceCharting hold years of history because they have been snapshotting licensed TCGPlayer / eBay sales data daily for years, and PriceCharting's API explicitly excludes history. The daily workflow above is how ours accumulates; a one-off backfill would need a licensed source and a product decision (see `docs/V1-WATCHLIST-AND-CHARTS.md`).
