...
(Design addendum, approved 6 Sep 2026 after ten simulated reviewers)
...
Built 6 Sep 2026 on `cursor/currency-health-f694` after Jib said proceed to build. No preview routes.
...
**About prices.** Costs never convert; a Portfolio S$ figure is the USD seed at the last European Central Bank table, dated; it is not a bank quote and not a Singapore ask.
...
2. Unpriced prints, named, each a link. Today: three (`EB04-061p3`, `OP05-115p1`, `OP17-118p2`). Print IDs in ink.
...
6. This switch is `US $` / `S$` only. Ringgit, rupiah, and translation wait on map 10.0 and on collectors in those markets asking.
