...
Draft 11 (`npm run wiki:belong`, `scripts/wiki-belong-refresh.mjs`). Same licence, User-Agent and 120 ms gap. Never an image. **Story index:** one parse of the wiki Story Arcs page (revid 2128192, 6 Sep 2026). 33 main arcs; each row is the title plus that page’s `Chapters (a-b)` and `Episodes (c-d)`. No plot paragraph is stored. Elbaph is open (`1126–`, `1156–`). Chapter pages are titles, not plots; official Viz / Toei / Crunchyroll synopses are not stored. **Fruits:** Char Box `dfename` / `dfname` / `dftype` on each mapped printed name in `lines.json` (188 asked). First displayed line only; text after `<br>` or in `<small>` is a later name and is dropped (Nika on Luffy is not stored). Localizer credits are stripped. `Imu` / `The Devil's Fruit` is not stored. 6 Sep 2026: **92 fruits** on 99 eaters. `seed:check` fails a missing licence, a stored plot paragraph, a localizer credit in a fruit name, or Imu as an eater. Wiki article URLs sit in the cache for provenance and are never rendered.
