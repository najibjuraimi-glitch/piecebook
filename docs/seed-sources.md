...
- **Upcoming sets (7.5, 5 Sep 2026): discovered automatically.** The TCGCSV step of `npm run seed:refresh` (also `-- --boxes-only`) reads every group in category 68 and, for any group not on the roster published within the last 30 days or in the future, its products; a group with a product named `… Booster Box` (not a Case) whose `presaleInfo.isPresale` is true becomes a roster row with `cardSeedStatus: pending`, TCGplayer's name (the box product name minus "Booster Box"), `presaleInfo.releasedOn` as `enReleased`, and the box's `tcgplayerProductId` / `tcgplayerGroupId` / `tcgplayerProductName`; its presale market price is snapshotted into `data/box-price-history.csv` like any roster box. Groups with no such box (a Deck Set, promos, release event cards) are logged as warnings and never added. **Provisional codes:** TCGCSV names carry no set code, so the refresh classifies the box by name (Extra Booster → `EB`, Premium Booster → `PRB`, otherwise `OP`), assigns the next number in that series from the roster and marks the row `codeProvisional: true`; each run probes `https://onepiece.limitlesstcg.com/cards/{code}` and a 200 whose title carries the code (`… (OP18) – Limitless One Piece`) flips the flag to false, a 404 leaves it pending; once the set's checklist seeds the row turns `ready`. The app never displays a provisional code (tile and page show the name alone) and `npm run seed:check` warns when a provisional row's release date has passed. First rows, 5 Sep 2026: `EB-05` *Extra Booster: One Piece Heroines Edition Vol.2* (group 24820, box 711385, 30 Oct 2026, pre-order market US $399.95) and `OP-18` *The Dominance of God* (group 24833, box 712901, 20 Nov 2026, US $399.72); Limitless returned 404 for both.
