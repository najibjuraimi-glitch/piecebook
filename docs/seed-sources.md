...
- **Box price feed (decision 3.1, 5 Sep 2026).** The roster is the only source of box prices. Cards reads each EN booster box by hand from the TCGPlayer product page in `usSource` (and an SG ask where one can be verified) about once a month and edits the roster row's `usMarketUsd` / `sgAskSgd` / `asOf` in a normal PR; the daily workflow never reads TCGPlayer (Limitless lists no sealed products). `npm run seed:refresh` (or `-- --boxes-only`) snapshots the roster's reads into `data/box-price-history.csv` (`set_code,as_of,us_market_usd,sg_ask_sgd,source=roster`, one row per set per read date; a same-date re-run replaces the row), so the box card can show movement between reads. `npm run seed:check` validates the file and warns when the newest read is more than 45 days old. First snapshot: 22 rows dated 2026-09-04.
