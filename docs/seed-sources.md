...
## Wiki belong rooms (`data/wiki/summaries.json`, `data/wiki/fruits.json`)

Draft 11 (`npm run wiki:belong`, `scripts/wiki-belong-refresh.mjs`). Same licence, User-Agent and 120 ms gap. Never an image. **Summaries:** one parse of the wiki Story Arcs page (revid 2128192, 6 Sep 2026). 33 main arcs; each row is the paragraph under `====[[Name]]====`, cleaned, never paraphrased, plus that page’s `Chapters (a-b)` and `Episodes (c-d)`. Elbaph is open (`1126–`, `1156–`). Chapter pages are titles, not plots; official Viz / Toei / Crunchyroll synopses are not stored. **Fruits:** Char Box `dfename` / `dfname` / `dftype` on each mapped printed name in `lines.json` (188 asked). First displayed line only; text after `<br>` or in `<small>` is a later name and is dropped (Nika on Luffy is not stored). Localizer credits are stripped. `Imu` / `The Devil's Fruit` is not stored. 6 Sep 2026: **92 fruits** on 99 eaters. `seed:check` fails a missing licence, a summary under six words, a localizer credit in a fruit name, or Imu as an eater. Wiki article URLs sit in the cache for provenance and are never rendered.
...
- **Weekly data health (9.1, 6 Sep 2026): named facts, named gaps.** `data/health.json` is written by `npm run seed:refresh` (or `-- --health-only`) from the files we already hold: print and file counts, the newest card / box / wiki dates, every seed row with a blank `market_usd`, and the exclusions the product already states (no story line, no EN box, no SG asks, no wiki images). The `/data` page is the weekly read. `seed:check` warns if the file is missing or more than 8 calendar days old. No percent, no traffic-light score, no `today` / `N days old`.
